...
@Service("smallSpotFee")
public class SmallSpotFee extends FeeDecorator {
    public SmallSpotFee(@Qualifier(value = "baseFeeCalculator")FeeCalculator feeCalculator) {
        super(feeCalculator);
    }

    public BigDecimal calculateFee(Vehicle vehicle) {
        return feeCalculator.calculateFee(vehicle).multiply(BigDecimal.valueOf(0.5));
    }
}
